...
import (
	...
	authstage "github.com/yelaco/gchess-tui/pkg/ui/stages/login/auth"
)
...
type FormStageModel struct {
	inputs    []textinput.Model
	submitted bool
	cursor    int
}

func NewFormStageModel() FormStageModel {
	m := FormStageModel{
...
func (m FormStageModel) Init() tea.Cmd {
	return nil
}

func (m FormStageModel) View() string {
...
func (m FormStageModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "ctrl+c", "esc":
			return m, tea.Quit
		case "enter":
			authStageModel := authstage.NewAuthStageModel(domains.Login{
...
func (m FormStageModel) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m *FormStageModel) moveCursor(msg tea.KeyMsg) {
	switch msg.String() {
	case "up", "shift+tab":
		if m.cursor > 0 {
			m.cursor--
